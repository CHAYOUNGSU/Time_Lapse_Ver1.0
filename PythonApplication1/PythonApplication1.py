...
class TimelapseApp:
    def __init__(self, root):
        ...
        self.image_frame = tk.Frame(root, bg="white", width=400, height=300, relief=tk.SOLID, bd=2, highlightbackground="black", highlightthickness=4)
    ...
    def start_timelapse(self):
        if not self.is_running:
            ...
            self.image_frame.config(highlightbackground="#0DFF7A")
            self.run_timelapse()
        else:
            self.is_running = False
            self.start_button.config(text="START", bg="#6BA8DF")
            self.image_frame.config(highlightbackground="black")
